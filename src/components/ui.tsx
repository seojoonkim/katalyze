...
export function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description?: string }) {
  return (
    <div className="mb-10 max-w-2xl">
      <p className="mb-3 text-sm uppercase tracking-[0.35em] text-[#F0B429]">{eyebrow}</p>
  ...
  return (
    <Link
      ...
      className={clsx(
        ...
        variant === "primary" && "bg-[#F0B429] text-black hover:bg-[#F0B429]",
        variant === "secondary" && "border border-white/15 bg-white/5 text-white hover:border-[#F0B429] hover:text-[#F0B429]",
...
export function ImageCard({ image, title, subtitle, description, className = "" }: { image: string; title: string; subtitle?: string; description?: string; className?: string }) {
  return (
    <div className={clsx("k-card overflow-hidden rounded-none", className)}>
      ...
      <div className="p-6">
        {subtitle ? <p className="text-sm uppercase tracking-[0.24em] text-[#F0B429]">{subtitle}</p> : null}
      ...
      {items.map((item, i) => (
        <div key={i} className="py-5">
          ...
          >
            ...
            <span className="ml-4 text-[#F0B429] text-xl">{open === i ? "−" : "+"}</span>
          </button>
          {open === i && (
...
export function StatCard({ value, suffix, label, ko }: { value: string; suffix?: string; label: string; ko?: string }) {
  return (
    <div className="border-l-2 border-[#F0B429] pl-6">
      <p className="font-display text-5xl text-white md:text-7xl">
        {value}<span className="text-3xl text-[#F0B429] md:text-5xl">{suffix}</span>
      </p>
      <p className="mt-2 font-outfit text-sm uppercase tracking-[0.3em] text-white/75">{label}</p>
      {ko && <p className="mt-1 font-outfit text-base text-[#2DD4BF]">{ko}</p>}
    </div>
  )
}
...
export function TierCard({ tier, ko, price, benefits, highlight, badge, color, cta }: {
  ...
  return (
    <div className={`relative flex flex-col border ${highlight ? 'border-[#F0B429]' : 'border-white/10'} bg-white/[0.03]`}>
      {badge && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#F0B429] px-4 py-1 text-base font-bold uppercase tracking-widest text-black">
          {badge}
        </div>
      )}
