...
export function TierCard({ tier, ko, price, benefits, highlight, badge, color, cta }: {
  ...
  return (
    <div className={`relative flex flex-col border ${highlight ? 'border-[#FABF13]' : 'border-white/10'} bg-white/[0.03]`}>
      {badge && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#FABF13] px-4 py-1 text-base font-bold uppercase tracking-widest text-black">
          {badge}
        </div>
      )}
