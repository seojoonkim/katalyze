      ...
      <AnimatedSection className="section-pad">
        <div className="container-shell">
          ...
          <div className="grid gap-12 md:grid-cols-2">
            {zones.map((zone) => (
              <div key={zone.slug} className="border-l border-[#FABF13] pl-8">
                {zone.subtitle ? <p className="text-sm uppercase tracking-[0.24em] text-[#FABF13]">{zone.subtitle}</p> : null}
                <h3 className="mt-3 text-3xl font-bold uppercase text-white">{zone.title}</h3>
                {zone.description ? <p className="mt-4 text-base leading-7 text-white/92">{zone.description}</p> : null}
              </div>
            ))}
          </div>
        </div>
